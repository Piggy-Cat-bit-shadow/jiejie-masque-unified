...
func TestNormalizeInterfacePrefix(t *testing.T) {
	...
	tests := []struct {
		...
		{"ipv4", &net.IPNet{IP: net.IPv4(10, 89, 0, 1).To4(), Mask: net.CIDRMask(16, 32)}, netip.MustParsePrefix("10.200.0.1/16"), true},
...
func TestNormalizedPrefixDoesNotMatchWrongIPv4(t *testing.T) {
	for _, tt := range []struct {
		...
		{"different-address", &net.IPNet{IP: net.IPv4(10, 89, 0, 2), Mask: net.CIDRMask(16, 32)}, netip.MustParsePrefix("10.200.0.1/16")},
		{"different-prefix-length", &net.IPNet{IP: net.IPv4(10, 89, 0, 1), Mask: net.CIDRMask(24, 32)}, netip.MustParsePrefix("10.200.0.1/16")},
	} {
		t.Run(tt.name, func(t *testing.T) {
			got, ok := normalizeInterfacePrefix(tt.addr)
			if !ok || got == tt.want {
