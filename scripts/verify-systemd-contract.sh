...
need() { rg -q -F "$2" "$1" || { echo "missing $2 in $1" >&2; exit 1; }; }
absent() { ! rg -q -F "$2" "$1" || { echo "forbidden $2 in $1" >&2; exit 1; }; }
...
for value in 'Type=simple' 'User=masque' 'EnvironmentFile=' 'check-config' 'LimitNOFILE=65536' 'TimeoutStopSec=5s' 'StateDirectory='; do need "$udp" "$value"; done
absent "$udp" 'CAP_NET_ADMIN'
absent "$udp" 'network-prepare'
check=$(rg -n -F 'check-config' "$ip" | head -1 | cut -d: -f1)
prepare=$(rg -n -F 'network-prepare' "$ip" | head -1 | cut -d: -f1)
