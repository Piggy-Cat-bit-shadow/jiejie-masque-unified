...
- F-402: CONNECT-IP configuration loading applies defaults, `KnownFields`,
  ...
  YAML loader. Omitted Session-NAT defaults are therefore accepted.
...
- F-405: release tag validation now verifies the remote GitHub annotated tag
  object and its exact commit target instead of trusting a local tag ref that
  `actions/checkout` may rewrite. Build and release jobs use the same validator
  and release assets still come from one validated workflow artifact.
